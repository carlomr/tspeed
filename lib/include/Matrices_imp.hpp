...
template<int N, typename Q, typename T> 
	...
	if(T::is_orthonormal)
	{
	    M.component(1).block(ie.id(),ie.id()) = rho*ie.detJ()*Eigen::MatrixXd::Identity(T::gdl, T::gdl);
	    invM.component(1).block(ie.id(),ie.id()) = 1./rho*1./ie.detJ()*Eigen::MatrixXd::Identity(T::gdl,T::gdl);
	    M.component(0).block(ie.id(),ie.id()) = rho*ie.detJ()*Eigen::MatrixXd::Identity(T::gdl, T::gdl);
	    invM.component(0).block(ie.id(),ie.id()) = 1./rho*1./ie.detJ()*Eigen::MatrixXd::Identity(T::gdl,T::gdl);
	}
    }
    template <int N, typename Q, typename T >
